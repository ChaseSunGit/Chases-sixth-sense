 ...
 * \brief       Library for interfacing with ICM42607 with DMA SPI and 
 *
 * \authors     Chase Sun
 * \bug
 */
...
//GLOBAL VARIABLES
ICM_Data ICM_holder;//The holder for IMU information. This declaration zeros all fields


// FUNCTIONS

/**
 * \brief Tool to write SPI command to a register of a specific ID
 *
 * \param reg //The spi register to write to, 8 bit
 * \param data //The data to write to, 8 bit
 * \return
 */
...
bool initICM(int IMU_ID, int reportFrequency) {
        ...
        //6-1. Change interrupt and read source to the FIFO to prevent partial reads from hanging the i2c line - 03/27/2026
        ...
        if (reportFrequency == 1600){
                ...
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, GYRO_CONFIG0_ICM,  0x27); // 400
        ...
        else if(reportFrequency == 200){
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, GYRO_CONFIG0_ICM,  0x48); //200
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG0_ICM, 0x28);
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, GYRO_CONFIG1_ICM,  0x01); //Bandwidth 34hz
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG1_ICM, 0x05);
        }
        ...
        //Setup FIFO
        //FIFO config1: streaming mode, no bypass, 00
        MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, FIFO_CONFIG1_ICM, 0x00);
        //FIFO config2: water mark set to 16 bytes. The FIFO will have 16 bytes every time a data pack is read
        //1byte header 6 byte accel 6 byte gyro 1 byte temp 2 byte timestamp, 0x10. The high byte can be 00
        MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, FIFO_CONFIG2_ICM, 0x10);
        MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, FIFO_CONFIG3_ICM, 0x00);

        //Configure FIFO configure register 5 on MREG1 to enable
        //This is a complex procedure since config 5 is on MREG1 which cannot be directly accessed. 
        //To do this, first turn power mode to non-idle and wait 10us,
        //Then write 0x00 to BLK_SEL_W, the register address to MADDR_W, and the value to M_W_ICM
        //Then there is a mandatory wait of 10us before moving on.

        //Set power mode
        // 0b0001 0000, power on oscillator even when accel and gyro are off for MREG1 access
        MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, PWR_MGMT0_ICM, 0x10);
        delayMicroseconds(10);
        //Write the MREG sequence
        //MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, BLK_SEL_W_ICM, 0x00); //0x00 for MREG1
        //MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, MADDR_W_ICM  , 0x01); //0x01 register for FIFO config 5
        //MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, M_W_ICM      , 0x23); //0b0010 0011 - trigger fifo int on every odr, enable accel and gyro
        //delayMicroseconds(20);//10us mandatory wait time, give it double for some overhead
        

        //Set up the interrupts
        //Set in pulse mode, push pull, active high, 0b0000 0011 0x03
        MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, INT_CONFIG_ICM, 0x03);

        //Write the MREG sequence for INT_CONFIG0 to disengage latch when reading the 
        //MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, BLK_SEL_W_ICM, 0x00); //0x00 for MREG1
        //MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, MADDR_W_ICM  , INT_CONFIG0_ICM); //0x01 register for FIFO config 5
        //MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, M_W_ICM      , 0x3F); //0b0011 1111 - clear latch on both status and data read
        //delayMicroseconds(20);//10us mandatory wait time, give it double for some overhead
        
        
        //Set int 2, 0b00001000, data ready - not used for now, using trigger to interface with mlx
        //MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, INT_SOURCE3_ICM, 0x08);
        
        if (IMU_ID == 0){ //Back IMU
                //Set int 1 to be FIFO crossing threshold - threshold should be set to 16 bytes above
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, INT_SOURCE0_ICM, 0x08); //04 for fifo, 08 for data ready
        }

        //Set power mode after MREG is accessed to enable accel and gyro operation
        // 0b0000 1111
...
bool initMLX(int IMU_ID) {
        ...
        MS5K_Data_Holder[IMU_ID].kalmanYaw.setQangle(0.01);
 ...
 */
 bool ICM_read_accel_gyro_lightweight(int IMU_ID, bool calibrationMode){
        int read_start_micros = micros();

        bool Should_we_flush_buffer = false;//Used to see if buffer is overflowing and needs a flush
        ...
        //The following block of code is used for reading from the data registers.
        //This has caused some blocking reads caused by a partial population of the data by the chip 
        //We will switch to reading from the FIFO instead
        ...
        int read_request_start = micros()-read_start_micros;
        ...
        int read_request = micros()-read_start_micros;
        

        
        
        int bytes_avail = MS5K_Data_Holder[IMU_ID].i2cBus->available();
        //Serial.print("Bytes returned: ");
        //Serial.println(bytes_avail);


        for (int i = 0; i < bytes_avail; ++i) {
                while (!MS5K_Data_Holder[IMU_ID].i2cBus->available()) {}
                buf[i] = MS5K_Data_Holder[IMU_ID].i2cBus->read();
        }
        ...
        //First check how much is left in the FIFO. If there is nothing, just skip and keep the last read
        
        //uint8_t FIFO_count_low  = MS5KRead(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, FIFO_COUNTL_ICM); //The fifo count must be read L then H
        //uint8_t FIFO_count_high = MS5KRead(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, FIFO_COUNTH_ICM);

        /*

        //int FifoCount = (int16_t)((FIFO_count_high << 8) | FIFO_count_low);
        int FifoCount = 16;
        //Serial.printf("FIFO_count %d\n",FifoCount);

        if (FifoCount < 16){
                //Fifo is empty, return
                return false;
        }
        else if(FifoCount > 32){
                //Serial.printf("WARNING: Buffer overflowing on IMU %d - Fifo count %d", IMU_ID, FifoCount);
                Should_we_flush_buffer = true;//Flush at the end of the function
        }

        //Next read the data in the fifo
        // Reads one Packet 3 entry:
        // byte 0  = header
        // bytes 1-6  accel
        // bytes 7-12 gyro
        // byte 13 = temp (ignored)
        // bytes 14-15 timestamp
        ...
        MS5K_Data_Holder[IMU_ID].i2cBus->write(FIFO_DATA_ICM);

        uint32_t read_fifo = micros()-read_start_micros;


        MS5K_Data_Holder[IMU_ID].i2cBus->endTransmission(true);

        int read_write_cmd = micros()-read_start_micros;

        delayMicroseconds(20);
        MS5K_Data_Holder[IMU_ID].i2cBus->requestFrom(static_cast<uint8_t>(MS5K_Data_Holder[IMU_ID].addr_IMU),static_cast<size_t>(16),true);

        int read_byte_avail = micros()-read_start_micros;
        ...
        int read_data = micros()-read_start_micros;
        ...
        // Unpack raw 16-bit values
        int16_t ax_raw = (int16_t)((buf[1]  << 8) | buf[2]);
        int16_t ay_raw = (int16_t)((buf[3]  << 8) | buf[4]);
        int16_t az_raw = (int16_t)((buf[5]  << 8) | buf[6]);
        
        int16_t gx_raw = (int16_t)((buf[7]  << 8) | buf[8]);
        int16_t gy_raw = (int16_t)((buf[9]  << 8) | buf[10]);
        int16_t gz_raw = (int16_t)((buf[11] << 8) | buf[12]);

        //buffer 13 is temp, 14-15 is time stamp. We skip those for now
        */
        ...
        // Gyro LSB per setting: 2000dps=16.4, 1000dps=32.8, 500dps=65.5, 250dps= 131.0
        const float GYRO_LSB_PER_DPS = 32.8f;      // for ±1000 dps
        const float G = 9.80665f;


        int read_conversion = micros()-read_start_micros;
        ...
        /*
        if (Should_we_flush_buffer){
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, SIGNAL_PATH_RESET_ICM, 0x04);//empty the buffer
                delayMicroseconds(2);//wait 2 microseconds as its required
                uint8_t flush_status = MS5KRead(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, SIGNAL_PATH_RESET_ICM);
                //Serial.printf("Flush status %d\n",flush_status);

        }
        */

        int read_finish = micros()-read_start_micros;

        //Serial.printf("%d,%d,%d,%d\n",read_request_start,read_request,read_conversion,read_finish);
 ...
 void Kalman_single_MS5K(int IMU_ID, float dt, int mode){
        ...
        */
        ...
        if(IMU_first_read){
                ...
                MS5K_Data_Holder[IMU_ID].kalmanYaw.setAngle(yawMag);
