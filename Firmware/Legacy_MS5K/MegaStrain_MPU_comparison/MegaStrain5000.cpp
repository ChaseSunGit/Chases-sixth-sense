...
bool MS5K_first_read = true;

uint32_t lastMicros_MS5K_MS5K_MS5K;//Used to time the kalman filter
...
bool initICM(int IMU_ID, int reportFrequency) {
        ...
        MS5K_Data_Holder[IMU_ID].valid_flag = 0;//assume imu is not used
        switch(IMU_ID){
                case 0://Back
                        MS5K_Data_Holder[IMU_ID].addr_IMU = 0x69;
                        MS5K_Data_Holder[IMU_ID].addr_MAG = 0x0F;
                        MS5K_Data_Holder[IMU_ID].addr_BMP = 0x47;
                        MS5K_Data_Holder[IMU_ID].i2cBus = &Wire2;
                        MS5K_Data_Holder[IMU_ID].ax_offset = 0;//These are factory offsets generated by the accel factory calibration function. Update when IMU is swapped
                        MS5K_Data_Holder[IMU_ID].ay_offset = 0;
                        MS5K_Data_Holder[IMU_ID].az_offset = 0;
                        break;
                case 1://L_shank
                        MS5K_Data_Holder[IMU_ID].addr_IMU = 0x68;
                        MS5K_Data_Holder[IMU_ID].addr_MAG = 0x0E;
                        MS5K_Data_Holder[IMU_ID].addr_BMP = 0x46;
                        MS5K_Data_Holder[IMU_ID].i2cBus = &Wire;
                ...
                case 3://R_shank
                        ...
                        MS5K_Data_Holder[IMU_ID].addr_MAG = 0x0E;
                        MS5K_Data_Holder[IMU_ID].addr_BMP = 0x46;
                        MS5K_Data_Holder[IMU_ID].i2cBus = &Wire1;
                        MS5K_Data_Holder[IMU_ID].ax_offset = 0;
                        MS5K_Data_Holder[IMU_ID].ay_offset = 0;
                        MS5K_Data_Holder[IMU_ID].az_offset = 0;
                        break;
        ...
        if (reportFrequency == 1600){
                ...
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG0_ICM, 0x45);
                ...
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG0_ICM, 0x46);
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, GYRO_CONFIG1_ICM,  0x01); //Bandwidth 180hz
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG1_ICM, 0x01);
        }
        else if(reportFrequency == 400){
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, GYRO_CONFIG0_ICM,  0x47); // 400
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG0_ICM, 0x47);
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, GYRO_CONFIG1_ICM,  0x02); //Bandwidth 72hz
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG1_ICM, 0x02);
        }
        else if(reportFrequency == 200){
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, GYRO_CONFIG0_ICM,  0x48); //200
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG0_ICM, 0x48);
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, GYRO_CONFIG1_ICM,  0x03); //Bandwidth 34hz
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG1_ICM, 0x03);
        }
        else if(reportFrequency == 100){
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, GYRO_CONFIG0_ICM,  0x49); //100
                MS5KWrite(IMU_ID, MS5K_Data_Holder[IMU_ID].addr_IMU, ACCEL_CONFIG0_ICM, 0x49);
 ...
 */
 void MegaStrain_bulk_initialization(int reportFrequency){
        for (int i = 0; i < (int)NUM_MS5K; i++){//The forloop initializes all 5 IMUs. It is fine if IMUs are not used, the code will just mark them invalid and skip them in reading data
                if(initICM(i,reportFrequency) == 0){Serial.printf("Failed to initialize IMU %d. Marked disabled, skipping\n", i);}
 ...
 */
 bool ICM_read_accel_gyro_lightweight(int IMU_ID, bool calibrationMode){
        ...
        const float ACCEL_LSB_PER_G = 8192.0f;     // for ±4g
 ...
 */
 void Kalman_single_MS5K(int IMU_ID, float dt){
        ...
        float rollAcc  = atan2(MS5K_Data_Holder[IMU_ID].ay, MS5K_Data_Holder[IMU_ID].az) * RAD_TO_DEG;
        float pitchAcc = atan2(-MS5K_Data_Holder[IMU_ID].ax, sqrt(MS5K_Data_Holder[IMU_ID].ay * MS5K_Data_Holder[IMU_ID].ay + MS5K_Data_Holder[IMU_ID].az * MS5K_Data_Holder[IMU_ID].az)) * RAD_TO_DEG;
        ...
        MS5K_Data_Holder[IMU_ID].roll = MS5K_Data_Holder[IMU_ID].kalmanRoll.getAngle(rollAcc, MS5K_Data_Holder[IMU_ID].gx, dt);

        MS5K_Data_Holder[IMU_ID].pitch = MS5K_Data_Holder[IMU_ID].kalmanPitch.getAngle(pitchAcc, MS5K_Data_Holder[IMU_ID].gy, dt);
 ...
 */
 void Kalman_batch_MS5K(){
        ...
        float dt = (currentMicros - lastMicros_MS5K_MS5K_MS5K) * 1e-6;
        Serial.printf("dt %.8f\n",dt);
        lastMicros_MS5K_MS5K_MS5K = currentMicros;

        for (int i = 0; i < (int)NUM_MS5K; i++){
                Kalman_single_MS5K(i,dt);
        }
        MS5K_first_read = false;//Only triggers once after the first read. Remove this line to disable kalman filter and only use accel for roll/pitch
 }
